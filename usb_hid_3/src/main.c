...
/**
  ...
  */
//				ADD PWN
TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;	//struktura do konfiguracji licznikow
TIM_OCInitTypeDef  TIM_OCInitStructure;			//struktura do zmiennej jasnosci
//				END PWN
int main(void)
{
...
//									ADD
  SystemInit();
  	...
  	// 				PWN ADD
  	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);	//uruchomienie zegara TIM4 [do ustawienia jasnosci diody 13]
  	//				PWN END
...
//									ENDADD
      //				ADD PWN
      //Implementacja diody 13 dla trybu PWN
  	...
  	TIM_Cmd(TIM3, ENABLE);											//uruchomienie zliczania
  	TIM_Cmd(TIM4, ENABLE);
      // 				END PWN
  ...
  while (1)
  {
	  ...
	  if(przyspieszenie_z>127)
	  {
	   ...
	   p_x = (acos((double)przyspieszenie_x / (double)60) * 180 / 3.14) - 90;
	   p_y = (acos((double)przyspieszenie_y / (double)60) * 180 / 3.14) - 90;
	   p_z = (acos((double)przyspieszenie_z / (double)60) * 180 / 3.14) - 90;
	    ...
	    */
	  //							ENDADD
	   //Edycja by wysylalo do konsoli
	   InBuffer[0] = (uint8_t)(p_x);
	   InBuffer[1] = (uint8_t)(p_y);
	  ...
	  {
		  licznik++;
		  TIM4->CCR2 = 333 - (licznik + 499/2) % 333; // set brightness
		  Delay(0xFFFF);
	  }
	  /*
